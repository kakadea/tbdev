...
function forum_mod_panel( $forumid, $topicid, $subject, $sticky, $locked ) {
  ...
  global $lang, $forum_mod_csrf;
  
  //attach_frame();
  $req_uri = '/forums.php?action=viewtopic&topicid=' . (int) $topicid;
  ...
  <table class='mod_option'>
    <tr>
      ...
      <td>
        ...
        <input type='hidden' name='csrf_token' value='" . htmlsafechars($forum_mod_csrf) . "' />
        <input type='hidden' name='topicid' value='" . (int) $topicid . "' />
        <input type='hidden' name='returnto' value='" . htmlsafechars($req_uri) . "' />
  ...
  while ($arr = mysql_fetch_assoc($res))
    if ($arr["id"] != $forumid && get_user_class() >= $arr["minclasswrite"])
      $htmlout .= "<option value='" . (int) $arr['id'] . "'>" . htmlsafechars($arr['name']) . "</option>\n";
